...
if task == "Sentiment Classification":
    ...
    if new_review_text.strip():
        ...
        if model_option == "Perceptron":
            with open('DL-ALGORITHMS\PP.pkl', 'rb') as file:
                model = pickle.load(file)
        elif model_option == "Backpropagation":
            with open('DL-ALGORITHMS\BP.pkl', 'rb') as file:
                model = pickle.load(file)
        elif model_option == "DNN":
            model = load_model('DL-ALGORITHMS\DP.keras')
        elif model_option == "RNN":
            model = load_model('DL-ALGORITHMS\RN.keras')
        elif model_option == "LSTM":
            model = load_model('DL-ALGORITHMS\LS.keras')
    ...
    if uploaded_file is not None:
        ...
        model = load_model('DL-ALGORITHMS\CN.keras')
